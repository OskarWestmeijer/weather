...
<div class="mx-auto flex w-full max-w-[1440px] flex-1 flex-col px-4 py-5">
	{#if overviewLocations.length === 0}
		<div
			class="flex min-h-[320px] flex-1 items-center justify-center rounded-2xl bg-[#f4f5f7] text-[15px] text-[#6b7480]"
		>
			No weather data available right now.
		</div>
	{:else}
		<div class="grid flex-1 gap-5 lg:min-h-0 lg:grid-cols-[1.35fr_1fr]">
			<!-- MAP -->
			<div class="flex min-h-0 flex-col gap-2.5 rounded-2xl bg-[#f4f5f7] p-3.5">
				<div class="min-h-[340px] flex-1 overflow-hidden rounded-[11px] bg-[#e7ebf1]">
					<WeatherMap
			...
			<!-- RANKED RAIL -->
			<div class="flex min-h-0 flex-col overflow-hidden rounded-2xl bg-[#f4f5f7]">
				<div class="flex items-baseline justify-between gap-3 px-[18px] pt-4 pb-2.5">
					<span class="truncate text-[15px] font-bold text-[#233554]">My favourite places</span>
					<span class="text-[13px] whitespace-nowrap text-[#6b7480]">coldest → warmest</span>
				</div>
				...
				<div class="flex min-h-0 flex-1 flex-col overflow-y-auto">
					{#each overviewLocations as location (location.locationId)}
						<a
							...
							class="grid min-h-[66px] flex-1 cursor-pointer grid-cols-[1fr_96px] items-center gap-3 border-t border-[#e6e9ef] px-[18px] py-[13px] transition"
							class:bg-[#dde5f1]={highlight === location.locationId}
							class:shadow-[inset_3px_0_0_#233554]={highlight === location.locationId}
